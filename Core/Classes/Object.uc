...
// Sean C. Dumas/Taldren added: Quaternion rotation definitions

struct native AxisAngle
{
	var float x, y, z, w;
};
/*
...
// 
//
// Returns "PC" if platform is PC, "PS2" for PS/2, and "XBOX" for XBox
native(2222) static final function String Platform();

// Returns true if we are running on a PC
native(2223) static final function bool IsPlatformPC();

// Returns true if we are running on a PS/2
native(2224) static final function bool IsPlatformPS2();

// Returns true if we are running on a XBox
native(2225) static final function bool IsPlatformXBox();
//
// KMY, end changes
//////////////////////
...
// SB/Taldren/4/12/03
//
function float GetUnitsFromMeters( float meters )
{
	return meters * 78.74;
}

function float GetMetersFromUnits( float units )
{
	return units / 78.74;
}
