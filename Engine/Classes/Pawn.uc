...
// 
//
var bool		bIsWalkingLocked;	// Hack to stop others from changing the walking.
//
// MT, end changes
//////////////////////
...
var vector	PreClippedVelocity;	// SCD$$$ Added
...
var() float      	BaseEyeHeight; 	// Base eye height above collision center.	-->  Modified:  Sean C. Dumas/Taldren
...
// Animation status
var name AnimStatus;			// !!!! Need to remove if not really used. (JP/Taldren)
...
var float MovementAnimRate[4];	// Added Sean C. Dumas/Taldren anim rate scaling for each direction SCD$$$ put this back in
...
function SetFiringRate( float rate );	//AFS/Taldren delay between shots
function SetDropItems( class<Pickup> item1, class<Pickup> item2, class<Pickup> item3 ); //AFS/Taldren
...
// 
//
	if ( ( bNewIsWalking != bIsWalking ) && ( ! bIsWalkingLocked ) )
//
// MT, end changes
//////////////////////
	{
...
function GiveAndEquipWeapon(string aClassName )
{
	local class<Weapon> WeaponClass;
	local Weapon NewWeapon;
	...
	// Equip weapon
	PendingWeapon = Weapon( FindInventoryType( WeaponClass ) );
	ChangedWeapon();
}

function GiveWeaponByClass( class<Weapon> WeaponClass )
{
	local Weapon NewWeapon;
...
function GiveAndEquipWeaponByClass( class<Weapon> WeaponClass )
{
	local Weapon NewWeapon;
...
/*	SetAnimStatus()
	...
	JP/Taldren: I think this was removed by Epic, but it is needed by HerdAnimal,
	Bot and ACTION_SetPosture classes.
*/
...
// 
//
	// NOTE removed reference to bPhysicsAnimUpdate because of a bug with the engine.
	// In a perfect system this function would defer to the setting of bPhysicsAnimUpdate
	// because the engine would provide a call to this function via an event.
	// However the engine alwasy returns zero velocity, so we must handle getting the call from the 
	// Engine by ignoring a zeroed value which is fine in this case because a fall of zero speed 
	// should be ignored anyway.  MFH
