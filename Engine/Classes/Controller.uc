...
// Added:  Sean C. Dumas/Taldren /TODO brought this back from older versions of Unreal
// not sure why it was removed or the what the new functionality is...
...
// SB, Taldren, 3/8/03
//
// Added this to disable firing, nano skills, using, etc
// while a character is being hit and playing a hit animation.
// Using a state would be problematic because you don't want to pop
// the player out of certain states, for example Driving.
//
var bool			fTakingAHit;
//
// End changes
////////////////
...
// SB, Taldren, 4/11/03
//
// Moved cloak stuff down to this level so AI and Players
...
var eCloakState	fCloakState;
var eCloakState fLastCloakState;
var byte		fMinCloakAlpha;
var byte		fCurCloakAlpha;
var bool		fCloakStateChanging;
var float		fCloakTimer;
const			kFullCloakTime	= 5;
const			kMinCloakAlpha	= 60;
var class<Emitter>	fCloakFXClass;
var name			fCloakFXBoneName;
var int			fCloakType;
//
// End changes
////////////////
...
native(518) final function Actor FindPathTo(vector aPoint, optional bool bClearPaths);	// Modified:  Sean C. Dumas/Taldren
...
function bool SetEnemy( Pawn Enemy );	// AFS/Taldren
function EquipCombatItemByString( string className, bool hidden ); // AFS/Taldren
...
// SB, Taldren, 4/11/03
//
function Cloak( int cloakType )
{
	local eCloakState newCloakState;
	
	switch( fCloakState )
		{
		case kUnCloaked:
		case kUnCloaking:
				fCloakType		= cloakType;
				newCloakState	= kCloaking;
			break;

		case kCloaked:
		case kCloaking:
				fCloakType		= 0;
				newCloakState	= kUnCloaking;
			break;

		default:
			log( "### Controller::Cloak() -- Error, bad cloak state" );
				return;
		}

	SetServerCloakState( newCloakState );
}


function SetServerCloakState( eCloakState CloakState )
{
	local Emitter	fx;
	local bool		DoFX;

	fLastCloakState = fCloakState;
	fCloakState		= CloakState;
	DoFX			= false;

	switch( fCloakState )
	{
	case kCloaking:
			fCloakStateChanging = true;
			DoFX				= true;

			if( fLastCloakState == kUnCloaked )
			{
				fCloakTimer	= kFullCloakTime;
			}
		break;
	...
	case kCloaked:
			fCloakStateChanging = false;
			fCurCloakAlpha = fMinCloakAlpha;
			Pawn.AdjustAlphaFade( fCurCloakAlpha );
		break;
	...
	// Now tell the client what state it should be in
	//
	if( Level.NetMode != NM_Standalone )
	{
        SetClientCloakState( fCloakState );
	}

	if( DoFX )
	{
		if( fCloakFXClass != None )
		{
            fx = Spawn( fCloakFXClass, Pawn,, Pawn.Location, Pawn.Rotation );
		}
		if( fCloakFXBoneName == '' )
		{
			fx.SetBase( Pawn );
		...
		{
            Pawn.AttachToBone( fx, fCloakFXBoneName );
		}
	}
}

simulated function SetClientCloakState( eCloakState CloakState )
{
	local Emitter	fx;
	local bool		DoFX;
...
simulated function UpdateCloak( float DeltaTime )
{
	if( fCloakStateChanging )
	{
		// Adjust time
		//
		fCloakTimer -= DeltaTime;


		// If the timer is up, the server will itself and 
		// the client to change states
		//
		if( fCloakTimer <= 0.0 )
		{
			fCloakTimer			= 0.0;
			...
			if( Role == ROLE_Authority )
			{
				if( fCloakState == kCloaking )
				{
					SetServerCloakState( kCloaked );
				}
				else if( fCloakState == kUnCloaking )
				{
					SetServerCloakState( kUnCloaked );
				}
			}
		...
		// Both server and client update the alpha for themselves during a change
		//
		if( fCloakState == kCloaking )
		{
			fCurCloakAlpha = MAX( fMinCloakAlpha, fCurCloakAlpha - ( DeltaTime * ( 255 /kFullCloakTime ) ) );
		}
		else if( fCloakState == kUnCloaking )
		{
			fCurCloakAlpha = MIN( 255, fCurCloakAlpha + ( DeltaTime * ( 255 /kFullCloakTime ) ) );
		}
...
simulated function int GetCloakLevel()
{
	if( fCloakState != kCloaked )
	{
		return -1;
	}
...
ignores SeePlayer, HearNoise, KilledBy;
	...
	function ServerReStartPlayer()
	{
		if ( Level.NetMode == NM_Client )
			return;
		Level.Game.RestartPlayer(self, true);
	}
}
