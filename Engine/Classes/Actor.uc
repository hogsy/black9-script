...
var array<FinalBlend>		ColorModSkins;		// Use this for Alpha fading effects Added Sean C. Dumas/Taldren	
var array<Material>			OldSkins;			// Use this to restore from Alpha fading effects Added Sean C. Dumas/Taldren
...
// 
//
var(Advanced) bool			bSafeSpawn;			// On current level, player cannot be killed by spawning over an existing player (or vice-versa)  They will be
												// moved appropriately.
//
// KMY, end changes
//////////////////////
...
var bool				  bReplicateSkinIndex; // moved here to save memory
// End Added

var class<LocalMessage> MessageClass;

//-----------------------------------------------------------------------------
// Savepoints (JP/Taldren 2/17/03)

enum Savepoint_Awareness
{
	SA_None,			// not relavent for savepoints (the default)
	SA_Location,		// save location only
	SA_Existence,		// save location if alive, remove from play if dead
	SA_CallMe			// Call SerializeSavepointData()
};

var(Advanced) Savepoint_Awareness SavepointAwareness;
var const int SavepointInternal; // internal variable for savepoint serialization data
...
// ESurfaceTypes is from 2003 ported by MH.  Comment below mentions Texture.uc but seems to have been moved to Material.uc
...
// actions for HUD icons
const	kHUDAction_None		= 00;
const	kHUDAction_PickUp	= 01;
const	kHUDAction_Use		= 02;
const	kHUDAction_Hack		= 04;
const	kHUDAction_Talk		= 08;
const	kHUDAction_Attack	= 16;
...
var bool		fShadeForHERA;
var bool		fShadeForNightscope;
...
// Added:  Sean C. Dumas/Taldren
	unreliable if ( (bReplicateSkinIndex || bNetInitial) && (Role==ROLE_Authority) && bNetDirty )
		SkinIndex;
// End Added
...
native function SetAnimRate( int Channel, float NewRate );	// Added:  Sean C. Dumas/Taldren
...
native final function CopyMaterialsToSkins();	// Added Sean C. Dumas/Taldren
...
///////////////////////////////////
// SB,Taldren,5/1/03
// Get the SurfaceType of a mesh or staticmesh
// from the SurfaceType of the mesh's material
native(556) final function ESurfaceTypes GuessSurfaceType();
// End changes
/////////////////////////
...
// Added:  Sean C. Dumas/Taldren
// skinning
native(600) final function SetSkinIndex(byte SkinNum);
...
// Called after loading a saved game (JP/Taldren).
//
event PostLoadGame();
...
// Use this for Alpha fading effects Added Sean C. Dumas/Taldren
function AdjustAlphaFade(byte Amount)
{
	...
	if ( Skins.Length == 0 )
	{
		...
		OldSkins.Length = Skins.Length;
	}

	for ( i = 0; i < Skins.Length && Skins[i] != none; i++ )
	{
		if ( i >= ColorModSkins.Length || ColorModSkins[i] == none )
		{
			ColorModSkins[i] = new(none) class'FinalBlend';
			ColorModSkins[i].FrameBufferBlending = FB_AlphaBlend;
			ColorModSkins[i].Material = new(none) class'ColorModifier';
			ColorModifier(ColorModSkins[i].Material).AlphaBlend = false;
			ColorModifier(ColorModSkins[i].Material).Material = new(none) class'Combiner';
			Combiner(ColorModifier(ColorModSkins[i].Material).Material).Material1 = Texture'Engine.WhiteSquareTexture';
			Combiner(ColorModifier(ColorModSkins[i].Material).Material).CombineOperation = CO_Use_Color_From_Material2;
			Combiner(ColorModifier(ColorModSkins[i].Material).Material).AlphaOperation = AO_Use_Alpha_From_Material1;
			shader = Shader(Skins[i]);
			if ( shader != none )
			{
				Combiner(ColorModifier(ColorModSkins[i].Material).Material).Material2 = shader.Diffuse;
			}
			else
			{
				Combiner(ColorModifier(ColorModSkins[i].Material).Material).Material2 = Skins[i];
			}
		}
		ColorModifier(ColorModSkins[i].Material).Color.A = Amount;
		if ( FinalBlend(Skins[i]) == None )
		{
			OldSkins[i] = Skins[i];
			Skins[i] = ColorModSkins[i];
		}
	}
}

// Use this for Undoing Alpha fading effects Added Sean C. Dumas/Taldren
function RemoveColorModifierSkins()
{
	local int i;

	if (OldSkins.Length > 0)
	{
		for ( i = 0; i < Skins.Length && Skins[i] != none; i++ )
		{
			// There is no delete in Unreal...
			// Let the New material(s) get garbage collected
			Skins[i] = OldSkins[i];
		}
		OldSkins.Length = 0;
	}
...
native final function SavepointDeadActor();
native final function SavepointVariable(Actor Savee, string identifier);
...
function int GetHUDActions( Pawn other )
{
	...
	if ( Pawn( self ) != None )
	{
		if ( Pawn( self ).Controller != None )
		{
			if ( Pawn( self ).Controller.AttitudeTo( other ) < ATTITUDE_Ignore )
			{
