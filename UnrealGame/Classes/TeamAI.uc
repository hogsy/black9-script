...
function bool PutOnDefense(Bot B)
{
	...
	if ( O != None )
	{
		if ( O.DefenseSquad == None )
		{
			...
			for ( i=0; i< PendingVehicles.Length; i++ )
				if ( (PendingVehicles[i] != None) && PendingVehicles[i].bDefensive )
				{
					O.DefenseSquad.SquadVehicle = PendingVehicles[i];
					PendingVehicles[i] = None;
					break;
				}
		}
		else
...
function PutOnOffense(Bot B)
{
	...
	if ( (AttackSquad == None) || (AttackSquad.Size >= AttackSquad.MaxSquadSize) )
	{
		...
		for ( i=0; i< PendingVehicles.Length; i++ )
			if ( (PendingVehicles[i] != None) && !PendingVehicles[i].bDefensive )
			{
				AttackSquad.SquadVehicle = PendingVehicles[i];
				PendingVehicles[i] = None;
				break;
			}
	}
