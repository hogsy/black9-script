...
class B9_BuyModKiosk extends B9_BaseKiosk;
...
function StockMods(array<string> items)
{
	...
	local class<B9_CharMod> ModClass;
	local string cname;
	local int price;
	local Pawn pawn;
			...
			if (item != None)
			{
				...
				if (cal != None)
				{
					while (cal.Calibration != None)
					{
						...
						if( B9_BodyModCalibration(cal) != None && cal.PawnName == pawn.Name &&
							B9_BodyModCalibration(cal).HoloMod.IsA(ItemClass.Name) )
						{
							item.Destroy();
							item = None;
							break;
...
function Purchase()
{
	...
	AddBodyModCalibration(pp, Stock[FrontIndex]);
