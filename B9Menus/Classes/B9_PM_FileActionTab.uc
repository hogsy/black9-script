...
class B9_PM_FileActionTab extends B9_PM_SharedTop;
...
function PostRender( canvas Canvas )
{
	...
	if (fSelItem == 0 && fByeByeTicks > 0.0f)
		theTex = fSingleLineFrameClickTex;
	else if (fSelItem == 0)
		theTex = fSingleLineFrameHiliteTex;
	else
		theTex = fSingleLineFrameTex;
	Canvas.SetPos( x + 128, y + 108 );
	Canvas.DrawTile( theTex, 256, 32, 0, 0, 256, 32 );
	Canvas.TextSize( fSaveStr, sx, sy );
	Canvas.SetPos( x + 256 - sx / 2, y + 116 );
	Canvas.DrawText( fSaveStr, false );

	if (fSelItem == 1 && fByeByeTicks > 0.0f)
		theTex = fSingleLineFrameClickTex;
	else if (fSelItem == 1)
		theTex = fSingleLineFrameHiliteTex;
	else
		theTex = fSingleLineFrameTex;
	Canvas.SetPos( x + 128, y + 140 );
	Canvas.DrawTile( theTex, 256, 32, 0, 0, 256, 32 );
	Canvas.TextSize( fLoadStr, sx, sy );
	Canvas.SetPos( x + 256 - sx / 2, y + 148 );
	Canvas.DrawText( fLoadStr, false );
	
	if (fSelItem == 2 && fByeByeTicks > 0.0f)
		theTex = fSingleLineFrameClickTex;
	else if (fSelItem == 2)
		theTex = fSingleLineFrameHiliteTex;
	else
		theTex = fSingleLineFrameTex;
	Canvas.SetPos( x + 128, y + 172 );
	Canvas.DrawTile( theTex, 256, 32, 0, 0, 256, 32 );
	Canvas.TextSize( fOptionsStr, sx, sy );
	Canvas.SetPos( x + 256 - sx / 2, y + 180 );
	Canvas.DrawText( fOptionsStr, false );

	if (fSelItem == 3 && fByeByeTicks > 0.0f)
		theTex = fSingleLineFrameClickTex;
	else if (fSelItem == 3)
		theTex = fSingleLineFrameHiliteTex;
	else
		theTex = fSingleLineFrameTex;
	Canvas.SetPos( x + 128, y + 204 );
	Canvas.DrawTile( theTex, 256, 32, 0, 0, 256, 32 );
	Canvas.TextSize( fQuitStr, sx, sy );
	Canvas.SetPos( x + 256 - sx / 2, y + 212 );
...
defaultproperties
{
	...
	fSaveStr="LOAD MAP ONE"
	fLoadStr="LOAD MAP TWO"
	fOptionsStr="LOAD MAP THREE"
	fQuitStr="QUIT TO MAIN MENU"
