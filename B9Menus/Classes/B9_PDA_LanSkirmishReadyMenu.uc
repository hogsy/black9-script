class B9_PDA_LanSkirmishReadyMenu extends B9_MenuPDA_Menu;

var B9MP_ServerBrowser fServerBrowser;
var localized string fOptionsLabel;
var localized string fReadyLabel;
var bool fRanOnce;
...
function UpdateMenu( float Delta )
{
	...
	if( fRanOnce == false )
	{
		...
		fDisplayItems[indexpoint]				= new(None)class'displayitem_ReadyItem';
log("fDisplayItems[indexpoint] ="$fDisplayItems[indexpoint]);	
		fDisplayItems[indexpoint].fLabel		= fReadyLabel;				
		displayitem_ReadyItem(fDisplayItems[indexpoint]).fNextMenuName = class'B9_PDA_MultiplayerCreateBrowseList';	
